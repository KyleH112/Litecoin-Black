...
#if defined(NDEBUG)
# error "LitecoinBlack cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x0435c660d4bf534d7da1a5c5cbda0f47d4c9eed9aa368a171b2a257924314853");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // LitecoinBlack: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "LitecoinBlack Signed Message:\n";
...
{
    // LitecoinBlack: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // LitecoinBlack: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // LitecoinBlack: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1525917039, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "Litecoin Black LTCB 05092018";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 100 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("046c24d6e12ead2dc749ba2eb0784ddb191c193cbab2032867945eeb4cbaf45da704c2cafe7e1edb58fdd9efca4dbb2d0e9e7844df7efb3dee0f864c19980cc4d5") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1525917039;
        ...
        assert(block.hashMerkleRoot == uint256("0xac4d652bf9e7af7813cef976f1aae8f5c780c7ea41a0d04d69bae12531f0a03b"));
...
unsigned char pchMessageStart[4] = { 0x94, 0xfb, 0x49, 0xb9 }; // LitecoinBlack: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("LitecoinBlackMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("LitecoinBlackMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static LitecoinBlackMiner(CWallet *pwallet)
{
    printf("LitecoinBlackMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("litecoinblack-miner");
    ...
    try { loop {
        ...
        printf("Running LitecoinBlackMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("LitecoinBlackMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&LitecoinBlackMiner, pwallet));
}
